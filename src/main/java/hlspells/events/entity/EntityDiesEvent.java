        ...
        if (event.getEntity() instanceof PlayerEntity) {
            if (!event.getSource().isBypassInvul()) {
                ...
                if (item.equals(ItemInit.TOTEM_OF_GRIEFING.get())) {
                    ...
                    com.heromotion.hlspells.network.NetworkManager.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new TotemPacket(new ItemStack(item)));
