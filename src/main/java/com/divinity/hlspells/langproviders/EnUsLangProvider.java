...
public class EnUsLangProvider extends LanguageProvider {
    ...
    protected void addTranslations() {
        ...
        EntityInit.ENTITIES.getEntries().forEach(e -> this.add(e.get(), WordUtils.capitalizeFully(BuiltInRegistries.ENTITY_TYPE.getKey(e.get()).toString().replace("hlspells:", "").replace("_", " "))));
        BlockInit.BLOCKS.getEntries().forEach(b -> this.add(b.get(), WordUtils.capitalizeFully(BuiltInRegistries.BLOCK.getKey(b.get()).toString().replace("hlspells:", "").replace("_", " "))));
        this.add("enchantment.hlspells.soul_bond", "Soul Bond");
        this.add("enchantment.hlspells.soul_syphon", "Soul Syphon");
        this.add("enchantment.hlspells.curse_of_sinking", "Curse Of Sinking");
        ItemInit.ITEMS.getEntries().stream().filter(i -> !(i.get() instanceof BlockItem)).forEach(i -> this.add(i.get(), WordUtils.capitalizeFully(BuiltInRegistries.ITEM.getKey(i.get()).toString().replace("hlspells:", "").replace("_", " "))));
        MenuTypeInit.MENUS.getEntries().forEach(m -> this.add(this.menuTypeRegistryName(m.get()), WordUtils.capitalizeFully(BuiltInRegistries.MENU.getKey(m.get()).toString().replace("hlspells:", "").replace("_", " "))));
